...
MySceneGraph.prototype.parseAnimations = function(animationsNode)
{
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        //type
        var animType = this.reader.getString(children[i], 'type');

        if (animType == null)
            return "no type defined for animation";
        if (animType != "linear" && animType != "circular" && animType != "bezier" && animType != "combo")
        ...
        if (animType == "combo")
        {
            ...
            var animationRefs = [];
            for(var k = 0; k < animationSpecs.length; k++) {

                if(animationSpecs[k].nodeName == "SPANREF") {
                    var idReference = this.reader.getString(animationSpecs[k], 'id');

                    if(idReference == null) {
                        return "An ID was not declared";
                    }

                    if(this.animations[idReference] == null){
                        return "That animation ID doesn't exist"
                    }

                    if(this.animations[idReference].type == "combo"){
                        return "Error - combo animation declared inside another combo animation"
                    }
                    animationRefs.push(idReference);
                }
                else{
                    this.onXMLMinorError("unknown tag name <" + animationSpecs[j].nodeName + ">");
                }
            }
            this.animations[id] = new MyComboAnimation(this.scene, id, animType, animationRefs);
        }
        ...
        if(animType == "circular")
        {
            ...
            this.animations[id] = new MyCircularAnimation(this.scene, id, animType, speed, center_x, center_y, center_z, radious, startAngle * DEGREE_TO_RAD, rotAngle * DEGREE_TO_RAD);
        }
        ...
        if (animType == "linear")
        {
            ...
            this.animations[id] = new MyLinearAnimation(this.scene, id, animType, speed, controlPointsAux);
        }
        ...
        if (animType == "bezier") {
             ...
             for (let j = 0; j < children[i].children.length; j++)
             {
                 ...
                 controlPointsAux.push(new Array(parseFloat(x), parseFloat(y), parseFloat(z)));
                 console.log('Control Points : ' + controlPointsAux);
             }
             this.animations[id] = new MyBezierAnimation(this.scene, id, animType, speed, controlPointsAux);
